...
def export_sales():
    ...
    try:
        ...
        if format_type == 'csv':
            # Export HTML instead of CSV for better readability
            sales_data = []
            total_amount = 0
            ...
            for sale in sales:
                ...
                sale_data = {
                    ...
                    'manager': sale.manager_username,
                    'service': get_service_display_name(sale.service_type),
                    'amount': sale.amount,
                    'payment': sale.payment_method or 'Non specificato',
                    'notes': sale.notes or ''
                }
                sales_data.append(sale_data)
                total_amount += sale.amount
            
            return render_template('reports/sales_export.html',
                                 sales=sales_data,
                                 total_amount=total_amount,
                                 store_name=database_manager.get_current_store_name(current_user),
                                 date_from=date_from,
                                 date_to=date_to,
        ...
        if format_type == 'csv':
            ...
            for customer in customers:
                customer_data = {
                    ...
                    'phone': customer.phone or 'Non specificato',
                    'email': customer.email or 'Non specificata',
                    'address': customer.address or 'Non specificato',
                    'birth_date': customer.birth_date,
                    'current_offer': customer.current_offer or 'Nessuna offerta',
                    'offer_expiry_date': customer.offer_expiry_date,
                    'notes': customer.notes or ''
                }
                customers_data.append(customer_data)
            
            return render_template('reports/customers_export.html',
